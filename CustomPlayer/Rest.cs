...
public partial class CustomPlayerPlugin
{
    ...
    {
        TShock.RestApi.Register(new SecureRestCommand("/custom/add/perm", args => RestAdd(args, "Perm"), "custom.admin"));
        TShock.RestApi.Register(new SecureRestCommand("/custom/add/prefix", args => RestAdd(args, "Prefix"), "custom.admin"));
        TShock.RestApi.Register(new SecureRestCommand("/custom/add/suffix", args => RestAdd(args, "Suffix"), "custom.admin"));

        TShock.RestApi.Register(new SecureRestCommand("/custom/list/perm", args => RestList(args, "Perm"), "custom.admin"));
        TShock.RestApi.Register(new SecureRestCommand("/custom/list/prefix", args => RestList(args, "Prefix"), "custom.admin"));
        TShock.RestApi.Register(new SecureRestCommand("/custom/list/suffix", args => RestList(args, "Suffix"), "custom.admin"));

        TShock.RestApi.Register(new SecureRestCommand("custom/wear/prefix", args => RestWear(args, "Prefix"), "custom.admin"));
        TShock.RestApi.Register(new SecureRestCommand("custom/wear/suffix", args => RestWear(args, "Suffix"), "custom.admin"));
    }
    private object RestAdd(RestRequestArgs args, string type)
    {
        var result = new RestObject("200");
        if (args.Parameters.Count() < 3)
            return SetError(result, "参数不足3个");
        var name = args.Parameters["name"];
        var value = args.Parameters["value"];
        var time = args.Parameters["time"];
        if (string.IsNullOrEmpty(name))
            return SetError(result, "name is empty");
        if (string.IsNullOrEmpty(value))
            return SetError(result, "value is empty");
        if (string.IsNullOrEmpty(time))
            return SetError(result, "time is empty");
        ...
        if (type == "Perm")
        {
            ...
            if (forever)
                result.Response = $"添加成功 玩家:{name} 权限:{value} 持续时间:永久";
            else
                result.Response = $"添加成功 玩家:{name} 权限:{value} 起始时间:{startTime} 结束时间:{endTime} 持续时间:{addTime}";
        }
        else
        {
            var addId = 0;
            ...
            if (TitleParse(type, ref value, null))
                return SetError(result, "get未找到值");
            ...
            if (forever)
                result.Response = $"添加成功 玩家:{name} {typeChinese}:{value} Id:{addId} 持续时间:永久";
            else
                result.Response = $"添加成功 玩家:{name} {typeChinese}:{value} Id:{addId} 起始时间:{startTime} 结束时间:{endTime} 持续时间:{addTime}";
        }
        return result;
    }
    private object RestList(RestRequestArgs args, string type)
    {
        ...
        else
        {
            ...
            bool isPrefix = type == nameof(CustomPlayer.Prefix);
            var typeChinese = isPrefix ? "前缀" : "后缀";
    ...
    private object RestWear(RestRequestArgs args, string type)
    {
        ...
        var isPrefix = type == nameof(CustomPlayer.Prefix);
        var typeChinese = isPrefix ? "前缀" : "后缀";
        int titleId, serverId;
        if (!int.TryParse(args.Parameters["id"], out titleId))
            return SetError(result, $"转换{typeChinese}Id失败");
        if (!int.TryParse(args.Parameters["serverid"], out serverId))
            return SetError(result, "转换ServerId失败");
        ...
        using (var reader = Utils.TitleQuery(type, name, titleId))
        {
            if (reader.Read())
            {
                ...
                SetError(result, $"玩家:{name} {typeChinese}Id:{titleId} 未找到");
        }
        return result;
    }
