...
public class TeleportRequest : TerrariaPlugin
{
	...
	private void OnInitialize(EventArgs e)
	{
		...
		Commands.ChatCommands.Add(new Command("tprequest.tpathere", TPAHere, "tpahere")
		{
			AllowServer = false,
		...
		Commands.ChatCommands.Add(new Command("tprequest.reload", ReloadTPR, "重读传送配置", "tprreload")
		{
			AllowServer = false,
			HelpText = "输入 /重读传送配置 会重新读取重读传送配置表."
		});
		SetupConfig();
        ServerApi.Hooks.GameUpdate.Register(this, (_) =>
        {
            TimerCoumt++;
            if (TimerCoumt % 60 == 0)
                OnElapsed();
	...
	private void TPA(CommandArgs e)
	{
		...
		TPRequests[e.Player.Index].timeout = tpConfig.Count + 1;
	...
	private void TPAHere(CommandArgs e)
	{
		...
		{
			e.Player.SendErrorMessage("玩家[{0}]没有被授予传送权限，发出传送请求无效！", list[0].Name);
			return;
		}
		for (int i = 0; i < TPRequests.Length; i++)
		{
