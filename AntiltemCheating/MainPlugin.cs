...
namespace AntiItemCheating
{
    ...
    public class MainPlugin : TerrariaPlugin
    {
        ...
        private void ReadConfig()
        {
            try
            {
                var flag = !File.Exists(IOPDConfigPath);
                if (flag)
                {
        ...
        {
            var flag = !IOPDConfig.哥布林一进行 && (args.npc.netID == 26 || args.npc.netID == 27 || args.npc.netID == 28 || args.npc.netID == 29 || args.npc.netID == 111);
            if (flag)
            {
                IOPDConfig.哥布林一进行 = true;
                ConfigFile.WriteConfig(IOPDConfig);
            }
            var flag2 = !IOPDConfig.旧日军团一 && (args.npc.netID == 564 || args.npc.netID == 565);
            if (flag2)
            {
                IOPDConfig.旧日军团一 = true;
                ConfigFile.WriteConfig(IOPDConfig);
            }
            var flag3 = !IOPDConfig.哥布林二进行 && args.npc.netID == 471;
            if (flag3)
            {
                IOPDConfig.哥布林二进行 = true;
                ConfigFile.WriteConfig(IOPDConfig);
            }
            var flag4 = !IOPDConfig.海盗进行 && (args.npc.netID == 212 || args.npc.netID == 213 || args.npc.netID == 214 || args.npc.netID == 215 || args.npc.netID == 216 || args.npc.netID == 491);
            if (flag4)
            {
                IOPDConfig.海盗进行 = true;
                ConfigFile.WriteConfig(IOPDConfig);
            }
            var flag5 = !IOPDConfig.日蚀一进行 && (args.npc.netID == 158 || args.npc.netID == 159 || args.npc.netID == 461);
            if (flag5)
            {
                IOPDConfig.日蚀一进行 = true;
                ConfigFile.WriteConfig(IOPDConfig);
            }
            var flag6 = !IOPDConfig.旧日军团二 && (args.npc.netID == 576 || args.npc.netID == 577);
            if (flag6)
            {
                IOPDConfig.旧日军团二 = true;
                ConfigFile.WriteConfig(IOPDConfig);
            }
            var flag7 = !IOPDConfig.日蚀二进行 && (args.npc.netID == 253 || args.npc.netID == 477);
            if (flag7)
            {
                IOPDConfig.日蚀二进行 = true;
                ConfigFile.WriteConfig(IOPDConfig);
            }
            var flag8 = !IOPDConfig.日蚀三进行 && (args.npc.netID == 460 || args.npc.netID == 463 || args.npc.netID == 466 || args.npc.netID == 467 || args.npc.netID == 468);
            if (flag8)
            {
                IOPDConfig.日蚀三进行 = true;
                ConfigFile.WriteConfig(IOPDConfig);
            }
            var flag9 = !IOPDConfig.旧日军团三 && args.npc.netID == 551;
            if (flag9)
            {
                IOPDConfig.旧日军团三 = true;
                ConfigFile.WriteConfig(IOPDConfig);
            }
            var flag10 = !IOPDConfig.火星进行 && (args.npc.netID == 381 || args.npc.netID == 382 || args.npc.netID == 383 || args.npc.netID == 384 || args.npc.netID == 385 || args.npc.netID == 386 || args.npc.netID == 389 || args.npc.netID == 390 || args.npc.netID == 392);
            if (flag10)
            {
        ...
        {
            return IOPDConfig.物品白名单.Count != 0 && Array.BinarySearch<int>(this.quickSort(IOPDConfig.物品白名单.ToArray(), 0, IOPDConfig.物品白名单.Count - 1), type, this.comparer) >= 0;
        }
        ...
        private void OnItemSlot(object? sender, GetDataHandlers.PlayerSlotEventArgs args)
        {
            ...
            if (!flag)
            {
                foreach (var itemChecker in this.validCheckers)
                {
                    var flag2 = itemChecker.Contains(args.Type) && !this.InWhiteList(args.Type) && IOPDConfig.封禁玩家;
                    if (flag2)
                    {
                    ...
                    {
                        var flag3 = !itemChecker.Contains((int) args.Type) || this.InWhiteList((int) args.Type) || IOPDConfig.封禁玩家;
                        if (!flag3)
                        {
                            ...
                            var flag4 = IOPDConfig.广播超进度 && ((this.LastType == args.Type && DateTime.Now > this.LastTime) || this.LastType != args.Type);
                            if (flag4)
                            {
        ...
        private static class CheckerLoader
        {
            ...
            internal static IItemChecker LoadBlackList()
            {
                ...
                foreach (var id in IOPDConfig.物品黑名单)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断哥布林
                    ? new DefaultItemChecker(() => GetJb("downedGoblins") || IOPDConfig.哥布林一进行)
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedGoblins || IOPDConfig.哥布林一进行);
                foreach (var id in IOPDConfig.哥布林一前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断鹿角怪
                    ? new DefaultItemChecker(() => GetJb("downedDeerclops"))
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedDeerclops);
                foreach (var id in IOPDConfig.鹿角怪前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断克眼
                    ? new DefaultItemChecker(() => GetJb("downedBoss1"))
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedBoss1);
                foreach (var id in IOPDConfig.克眼前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断虫脑
                ...
                foreach (var id in IOPDConfig.虫脑前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断旧日一
                    ? new DefaultItemChecker(() => GetJb("旧日军团一"))
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedBoss2 & IOPDConfig.旧日军团一);
                foreach (var id in IOPDConfig.旧日一前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断蜂后
                    ? new DefaultItemChecker(() => GetJb("downedQueenBee"))
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedQueenBee);
                foreach (var id in IOPDConfig.蜂后前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断骷髅
                    ? new DefaultItemChecker(() => GetJb("downedBoss3"))
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedBoss3);
                foreach (var id in IOPDConfig.骷髅前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断肉墙
                    ? new DefaultItemChecker(() => GetJb("hardMode"))
                    : (IItemChecker) new DefaultItemChecker(() => Main.hardMode);
                foreach (var id in IOPDConfig.肉墙前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断哥布林二
                    ? new DefaultItemChecker(() => GetJb("哥布林二进行"))
                    : (IItemChecker) new DefaultItemChecker(() => Main.hardMode & IOPDConfig.哥布林二进行);
                foreach (var id in IOPDConfig.哥布林二前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断海盗
                    ? new DefaultItemChecker(() => GetJb("海盗进行"))
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedPirates || IOPDConfig.海盗进行);
                foreach (var id in IOPDConfig.海盗前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断日蚀一
                    ? new DefaultItemChecker(() => GetJb("日蚀一进行"))
                    : (IItemChecker) new DefaultItemChecker(() => Main.hardMode & IOPDConfig.日蚀一进行);
                foreach (var id in IOPDConfig.日蚀一前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断旧日二
                    ? new DefaultItemChecker(() => GetJb("旧日军团二"))
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedMechBossAny & IOPDConfig.旧日军团二);
                foreach (var id in IOPDConfig.旧日二前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断机械眼
                    ? new DefaultItemChecker(() => GetJb("downedMechBoss2"))
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedMechBoss2);
                foreach (var id in IOPDConfig.机械眼前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断机械虫
                    ? new DefaultItemChecker(() => GetJb("downedMechBoss1"))
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedMechBoss1);
                foreach (var id in IOPDConfig.机械虫前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断机械骷髅
                    ? new DefaultItemChecker(() => GetJb("downedMechBoss3"))
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedMechBoss3);
                foreach (var id in IOPDConfig.机械骷髅前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断三王前
                ...
                foreach (var id in IOPDConfig.三王前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断日蚀二
                    ? new DefaultItemChecker(() => GetJb("日蚀二进行"))
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedMechBoss1 & NPC.downedMechBoss2 & NPC.downedMechBoss3 & IOPDConfig.日蚀二进行);
                foreach (var id in IOPDConfig.日蚀二前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断猪鲨前
                    ? new DefaultItemChecker(() => GetJb("downedFishron"))
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedFishron);
                foreach (var id in IOPDConfig.猪鲨前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断日蚀三
                    ? new DefaultItemChecker(() => GetJb("日蚀三进行"))
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedPlantBoss & IOPDConfig.日蚀三进行);
                foreach (var id in IOPDConfig.日蚀三前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断石巨人
                    ? new DefaultItemChecker(() => GetJb("downedGolemBoss"))
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedGolemBoss);
                foreach (var id in IOPDConfig.石巨人前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断旧日三
                    ? new DefaultItemChecker(() => GetJb("旧日军团三"))
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedGolemBoss & IOPDConfig.旧日军团三);
                foreach (var id in IOPDConfig.旧日三前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            internal static IItemChecker LoadBeforeMartian()
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断虫脑
                    ...
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedMartians || IOPDConfig.火星进行);
                foreach (var id in IOPDConfig.外星人前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断所有柱子
                ...
                foreach (var id in IOPDConfig.所有柱子前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
            ...
            {
                var itemChecker = IOPDConfig.连接远程服务器判断进度 && IOPDConfig.是否启用远程进度判断月总
                    ? new DefaultItemChecker(() => GetJb("downedMoonlord"))
                    : (IItemChecker) new DefaultItemChecker(() => NPC.downedMoonlord);
                foreach (var id in IOPDConfig.月总前禁物品)
                {
                    itemChecker.Add(id);
                }
                return itemChecker;
