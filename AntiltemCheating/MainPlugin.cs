...
public class MainPlugin : TerrariaPlugin
{
    ...
    private readonly short LastType;
    ...
    public override void Initialize()
    {
        ...
        GetDataHandlers.PlayerSlot += this.OnItemSlot;
        this.ReadConfig();
        var chatCommands = Commands.ChatCommands;
        var val = new Command("重读超进度物品权限", new CommandDelegate(this.CMD), new string[1] { "重读超进度物品限制" })
        {
    ...
    {
        return type == 0 || IOPDConfig.物品白名单.Exists(f => f == type);
    ...
    private void OnItemSlot(object sender, GetDataHandlers.PlayerSlotEventArgs args)
    {
        ...
        if (!this.validCheckers.Any((IItemChecker f) => f.Contains(args2.Type)) || this.InWhiteList(args2.Type))
        {
            return;
        }
        if (IOPDConfig.封禁玩家)
        {
            this.LogDetected(args2);
            var text = $"[i:{args2.Type}]如果你是清白的,就请来解释一通";
            val.Ban(text, "Server");
            args2.Type = 0;
            args2.Handled = true;
            val.Disconnect(text);
        ...
        val.SendErrorMessage($"检测到玩家背包里持有超进度物品: [i:{args2.Type}] 并进行清理！");
        var text2 = "检测到玩家: " + val.Name + " 背包里持有超进度物品 " + Lang.GetItemNameValue(args2.Type);
        ...
        args2.Stack = 0;
        val.SendData(PacketTypes.PlayerSlot, null, val.Index, args2.Slot);
    }
    ...
    {
        using var val = DbExt.QueryReader(TShock.DB, "SELECT * FROM synctable WHERE `key`=@0", new object[1] { key });

        return val.Read() && val.Get<bool>("value");
    }
}
