	...
	private static void SwitchCmd(CommandArgs args)
	{
		...
		case PlayerState.AddingCommands:
		{
			...
			case "tj":
			{
				...
				player.SendSuccessMessage("成功添加: {0}".SFormat(text));
				SwitchCommands.database.Write(Database.databasePath);
				break;
			}
			case "list":
			...
			case "lb":
			{
				...
				{
					player.SendMessage("({0}) ".SFormat(i) + data.commandList[i], Color.Yellow);
					SwitchCommands.database.Write(Database.databasePath);
				}
				break;
			}
			...
			case "sc":
			{
				...
				player.SendSuccessMessage("成功删除了第{1}条指令：{0}。".SFormat(text2, result3));
				SwitchCommands.database.Write(Database.databasePath);
				break;
			}
			case "冷却":
			...
			case "lq":
			{
				...
				player.SendSuccessMessage("冷却时间已设置为 {0} 秒".SFormat(result));
				SwitchCommands.database.Write(Database.databasePath);
				break;
			}
			case "权限忽略":
			...
			case "qxhl":
			{
				...
				player.SendSuccessMessage("是否忽略玩家权限设置为: {0}.".SFormat(result2));
				SwitchCommands.database.Write(Database.databasePath);
				break;
			}
			case "取消":
			...
			case "wc":
			{
				...
				player.SendSuccessMessage("设置成功的开关位于 X： {0}， Y： {1}".SFormat(data2.X, data2.Y));
				...
				player.SetData("PlayerState", PlayerState.None);
				player.SetData("SwitchPos", default(Vector2));
				player.SetData("CommandInfo", new CommandInfo());
				SwitchCommands.database.Write(Database.databasePath);
				return;
			}
			...
			player.SetData("CommandInfo", data);
			break;
		}
		}
	}
