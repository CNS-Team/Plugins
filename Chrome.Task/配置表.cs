        ...
        try
        {
            ...
            if (!File.Exists(path))
            {
                ...
                任务系统.配置.任务.Add(new()
                {
                    ...
                    任务简述 = "说明任务系统插件的注意事项",
                    任务详细描述 = "",
                    完成指令 = { "命令如下 /查询任务 /接受任务 /任务列表 /完成任务 /reload", "任务ID不能为0,0视为完成所有主线任务,玩家初始任务ID为1", "完成指令:server执行,{0}会被替换为玩家名字", "解锁任务:直接接取数组里的主线任务，如果有多个主线任务需要玩家自己选择。支线任务变为可接取。如果没有主线任务，会视为完成所有主线任务。"
                        ...
                        , "玩家/接取任务 时会判断 进度限制 。所以单个主线任务时,会忽略进度限制", "不能通过支线任务接受主线任务", "任务要求:", "任务类型0 消耗物品完成任务 目标参数 \"netID,stack\"",
                        "任务类型1 背包中是否有指定物品 目标参数 \"netID\"", "任务类型2 是否击杀指定NPC  目标参数 \"npcID,stack\"", "任务类型3 到达指定地图区域  目标参数 \"X,Y,偏移值\"",
                        "任务类型4 穿戴或拿起指定装备 目标参数 \"netID\"", "任务类型5 拥有指定buff //目标参数 \"buffID\"",
                        "任务类型6 对话指定NPC 目标参数 \"npcID\" 插件只记录玩家的上一个对话NPC,所以该任务要求每个任务只能有一个,否则玩家无法完成任务",
                        "任务类型7 击杀指定NPC数量达到目标 目标参数 \"npcID,npcID,npcID.......,stack\"",
                        "任务类型8 玩家达到指定等级及以上 目标参数 \"int\"",
                        "任务类型9 玩家为指定职业 目标参数 \"name\""
                    },
                    解锁任务 = { },
                    任务要求 = { new() { 要求ID = 0, 目标参数 = "4956,1" } }
                });
