...
public class Data
{
	...
	{
		bool result = false;
		QueryResult val = DbExt.QueryReader(db, "select Username,Password,UUID,Usergroup,Registered,LastAccessed,KnownIPs from Users where Username='" + name + "'", Array.Empty<object>());
		try
		{
			if (val.Read())
			{
