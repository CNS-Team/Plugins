...
namespace DataSync
{
    ...
    public class Plugin : TerrariaPlugin
    {
        ...
        private static void EnsureTable()
        {
            ...
            using (var result = TShock.DB.QueryReader("SELECT * FROM synctable WHERE `key`=@0", "hardMode"))
            {
                if (!result.Read())
                {
                    TShock.DB.Query("INSERT INTO synctable (`key`, `value`) VALUES " + "(@0, @1);", "hardMode", "false");
        ...
        public static void UploadProgress(string key, string value)
        {
        ...
        public static void LoadProgress()
        {
            ...
            foreach (var kvp in LoadFromDb())
            {
                ...
                else if (配置.进度同步肉墙 && kvp.Key == "hardMode")
                {
