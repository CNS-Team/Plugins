...
public class KeyChanger : TerrariaPlugin
{
    ...
    private KeyTypes?[] Exchanging { get; set; }
    ...
    private void onGetData(GetDataEventArgs e)
    {
        ...
        using var binaryReader = new BinaryReader(new MemoryStream(e.Msg.readBuffer, e.Index, e.Length));
        ...
        var item = key.Items[random.Next(0, key.Items.Count)];
        ...
        else
        {
            ...
            var itemById2 = TShock.Utils.GetItemById((int) key.Type);
        ...
        else if (args.Parameters.Count > 0)
        {
            switch (args.Parameters[0].ToLower())
            {
                ...
                case "change":
                {
                    ...
                    if (!player.TPlayer.inventory.Any(i => i.netID == (int) key.Type))
                    {
