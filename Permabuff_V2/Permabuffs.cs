...
public class Permabuffs : TerrariaPlugin
{
    ...
    public override void Initialize()
    {
        ...
        RegionHooks.RegionEntered += this.OnRegionEnter;
        GeneralHooks.ReloadEvent += this.PBReload;
    }
    ...
    public void OnInitialize(EventArgs args)
    {
        ...
        update.Elapsed += this.OnElapsed!;

        Commands.ChatCommands.Add(new Command("pb.use", this.PBuffs, "permabuff") { AllowServer = false, HelpText = "给你一个永久Buff." });
        Commands.ChatCommands.Add(new Command("pb.check", this.PBCheck, "buffcheck") { HelpText = "列出玩家有效的永久Buff." });
        Commands.ChatCommands.Add(new Command("pb.give", this.PBGive, "gpermabuff") { HelpText = "给一个玩家永久Buff." });
        Commands.ChatCommands.Add(new Command("pb.region", this.PBRegion, "regionbuff"));
        Commands.ChatCommands.Add(new Command("pb.global", this.PBGlobal, "globalbuff"));
        Commands.ChatCommands.Add(new Command("pb.use", this.PBClear, "clearbuffs") { HelpText = "清除所有Buff." });
    }
    ...
    {
        var rb = config.regionbuffs.FirstOrDefault(p => p.regionName == args.Region.Name && p.buffs.Count > 0)!;
    ...
    private void OnElapsed(object sender, ElapsedEventArgs args)
    {
        for (var i = 0; i < TShock.Players.Length; i++)
        {
            ...
            {
                var rb = config.regionbuffs.FirstOrDefault(p => TShock.Players[i].CurrentRegion.Name == p.regionName && p.buffs.Count > 0)!;

                if (rb != null)
                {
                    foreach (var kvp in rb.buffs)
                    {
    ...
    private void PBuffs(CommandArgs args)
    {
        ...
        var buff = string.Join(" ", args.Parameters);

        // Get buff type by name
        if (!int.TryParse(args.Parameters[0], out bufftype))
        {
            var bufftypelist = TShock.Utils.GetBuffByName(buff);
        ...
        var playerid = args.Player.Account.ID;
    ...
    private void PBRegion(CommandArgs args)
    {
        ...
        if (args.Parameters[0].Equals("add", StringComparison.CurrentCultureIgnoreCase))
        {
            ...
            var buffinput = args.Parameters[2];
            if (args.Parameters.Count != 4)
            {
                args.Player.SendErrorMessage("参数无效: {0}regionbuff <add/del> <区域名> <Buff名/BuffID> [持续时间]", args.Silent ? TShock.Config.Settings.CommandSilentSpecifier : TShock.Config.Settings.CommandSpecifier);
                return;
            }
            var durationinput = args.Parameters[3];
            var bufftype = -1;
            ...
            {
                var bufflist = TShock.Utils.GetBuffByName(buffinput);
            ...
            for (var i = 0; i < config.regionbuffs.Length; i++)
            {
            ...
            if (!found)
            {
                var temp = config.regionbuffs.ToList();
    ...
    private void PBClear(CommandArgs args)
    {
        if (args.Parameters.Count == 1 && (args.Parameters[0] == "*" || args.Parameters[0].Equals("all", StringComparison.CurrentCultureIgnoreCase)))
        {
            ...
            foreach (var kvp in DB.PlayerBuffs)
            {
