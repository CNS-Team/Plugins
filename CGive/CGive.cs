...
public class CGive
{
	...
	public bool Execute()
	{
			...
			if (list.Count > 0 || Executer.ToLower() == "server")
			{
				...
				foreach (TSPlayer val in array2)
				{
					if (val != null && val.Active)
					{
						...
						{
							Commands.HandleCommand((TSPlayer)(object)TSPlayer.Server, cmd.Replace("name", val.Name));
						}
						else
						{
							Commands.HandleCommand(list[0], cmd.Replace("name", val.Name));
						}
		...
		if (list2.Count > 0)
		{
			...
			if (list3.Count > 0 || Executer.ToLower() == "server")
			{
				if (Executer.ToLower() == "server")
				{
					Commands.HandleCommand((TSPlayer)TSPlayer.Server, cmd.Replace("name", who));
				}
				else
				{
	...
	public static List<CGive> GetCGive(string who)
	{
		...
		QueryResult val = DbExt.QueryReader(TShock.DB, "select executer,cmd,who,id from CGive where who='" + who + "' or who=='-1'", Array.Empty<object>());
		try
		{
			while (val.Read())
			{
				list.Add(new CGive
				{
					Executer = val.Reader.GetString(0),
					cmd = val.Reader.GetString(1),
					who = who,
					id = val.Reader.GetInt32(3)
				});
			}
		}
		finally
	...
	{
		QueryResult re = DbExt.QueryReader(TShock.DB, "select executer,cmd,who,id from CGive", Array.Empty<object>());
		try
		{
			while (re.Read())
			{
