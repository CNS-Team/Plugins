           ...
           GetNPCFilelds().ForEach(f =>
            {
                if (f.Key == "血肉墙")
                {
                    Progress.Add(f.Key, Convert.ToBoolean(f.Value.GetValue(new Main())));
                }
                else
                {
                    Progress.Add(f.Key, Convert.ToBoolean(f.Value.GetValue(new NPC())));
                }
            });
            return Progress;
            ...
            //    { "世界吞噬怪", NPC.downedBoss2 },
            //    { "克苏鲁之脑", NPC.downedBoss2 },
            //    { "骷髅王", NPC.downedBoss3 },
            //    { "独眼巨鹿", NPC.downedDeerclops },
            ...
            //    { "月球领主", NPC.downedMoonlord },
            //    { "雪人军团", NPC.downedFrost },
            //    { "哥布林军队", NPC.downedGoblins },
            //    { "海盗入侵", NPC.downedPirates },
            //    { "南瓜月", NPC.downedHalloweenKing },
            //    { "火星暴乱", NPC.downedMartians },
            //    { "霜月", NPC.downedChristmasIceQueen }
            //};
        }
