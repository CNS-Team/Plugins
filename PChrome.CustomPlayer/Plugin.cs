...
{
    public override string Name => "PChrome.CustomPlayer";

    public Plugin(Main game) : base(game)
    {
    }

    private static readonly Dictionary<string, string[]> permCache = new Dictionary<string, string[]>();
    ...
    private static void OnChat(ServerChatEventArgs args)
    {
        ...
        var text = args.Text;
        foreach (var keyValuePair in ChatManager.Commands._localizedCommands)
        {
        ...
        if ((text.StartsWith(TShock.Config.Settings.CommandSpecifier) || text.StartsWith(TShock.Config.Settings.CommandSilentSpecifier)) && !string.IsNullOrWhiteSpace(text[1..]))
        {
            try
            {
                args.Handled = true;
        ...
        if (!TShock.Config.Settings.EnableChatAboveHeads)
        {
            ...
            var flag = PlayerHooks.OnPlayerChat(tsplayer, args.Text, ref text);
            args.Handled = true;
            if (flag)
            {
                return;
        ...
        else
        {
            ...
            var text2 = string.Format("<{0}> {1}", string.Format(TShock.Config.Settings.ChatAboveHeadsFormat, new object[]
            {
                tsplayer.Group.Name,
                string.IsNullOrEmpty(prefix) ? tsplayer.Group.Prefix : prefix,
                tsplayer.Name,
