...
namespace 称号插件
{
    ...
    public class 称号插件 : TerrariaPlugin
    {
        ...
        private void 重载(CommandArgs args)
        {
            ...
            {
                TSPlayer.Server.SendErrorMessage($"[Chrome.Title]配置文件读取错误");
            }
        }
        ...
        public static void Add(string 玩家用户名, string? 前前缀, string? 前缀, string? 角色名, string? 后缀, string? 后后缀)//添加称号
        {
            ...
            {
                配置.称号设置.Add(new Config.名称() { 玩家用户名 = 玩家用户名, 前前缀 = 前前缀, 前缀 = 前缀, 角色名 = 角色名, 后缀 = 后缀, 后后缀 = 后后缀 });
            }
            File.WriteAllText(path, JsonConvert.SerializeObject(配置, Formatting.Indented));
        }
        ...
        private static void 聊天(ServerChatEventArgs args)
        {
            ...
            if (配置.称号设置.Exists(a => a.玩家用户名 == plr.Name))//是否有称号，称号优先级最高
            {
                ...
                发送聊天(args, args.Text, 前前缀, 前缀, 角色名, 后缀, 后后缀, z.颜色);
                //c.前前缀 = ""; c.前缀 = null; c.后缀 = null; c.后后缀 = ""; c.角色名 = null;//发生消息成功后，称号信息归位

            }
            else//如果没称号
            {
                //var c = new Config.聊天信息() { };
                if (称号信息.TryGetValue(plr.Name, out var c))//是否有玩家信息
                {
                    string 前前缀;
                    string 前缀;
                    string 角色名;
        ...
        public static bool 发送聊天(ServerChatEventArgs args, string 聊天内容, string 前前缀, string 前缀, string 角色名, string 后缀, string 后后缀, Color? 颜色)
        {
            ...
            if (聊天检测(args))
            {
                ...
                {
                    TSPlayer.All.SendMessage(发送内容, 颜色.Value.R, 颜色.Value.G, 颜色.Value.B);
                    TSPlayer.Server.SendMessage(发送内容, 颜色.Value.R, 颜色.Value.G, 颜色.Value.B); //经过实践，[c/FFFFFF:],[i:]仍然生效
                }
