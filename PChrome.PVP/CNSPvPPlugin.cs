...
namespace PChrome.PVP
{
    ...
    public class CNSPvPPlugin : LazyPlugin
    {
        ...
        public override void Initialize()
        {
            ...
            GetDataHandlers.NewProjectile += this.OnProjCreate;
            GetDataHandlers.ProjectileKill += this.OnProjDestory;
            ...
            this.SpecialProj = new Dictionary<int, Config.WeaponDebuffInfo>();
        }
        ...
        private void OnPlayerDamage(object o, GetDataHandlers.PlayerDamageEventArgs args)
        {
            ...
            if (player == null || !this.SpecialProj.TryGetValue(
                    args.PlayerDeathReason.SourceProjectileType != null ?
                    Main.projectile[args.PlayerDeathReason.SourceProjectileType.Value].identity : -1, out var value) ||
                args.PlayerDeathReason._sourceItemType != value.ID)
            {
                return;
            }
        ...
        private void OnProjCreate(object o, GetDataHandlers.NewProjectileEventArgs args)
        {
            ...
            if (weaponDebuffInfo != null && !this.SpecialProj.ContainsKey(args.Identity))
            {
                this.SpecialProj.Add(args.Identity, weaponDebuffInfo);
            }
        }

        private void OnProjDestory(object o, GetDataHandlers.ProjectileKillEventArgs args)
        {
            if (this.SpecialProj.ContainsKey(args.ProjectileIndex))
            {
                this.SpecialProj.Remove(args.ProjectileIndex);
            }
        }
