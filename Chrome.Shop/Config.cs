...
public class Config
{
    public static void GetConfig()
    {
        try
        {
            ...
            {
                FileTools.CreateIfNot(Path.Combine("tshock/Chrome.Shop.json"), JsonConvert.SerializeObject(shop.ShopPlugin.配置, Formatting.Indented));
                shop.ShopPlugin.配置 = JsonConvert.DeserializeObject<Config>(File.ReadAllText(Path.Combine("tshock/Chrome.Shop.json")));
                shop.ShopPlugin.配置?.Shops.Add(new() { 价格 = 1000, 显示名 = "木材", 进度限制 = new() { "史莱姆王" }, 商品 = new() { new() { 数量 = 100, 前缀 = 0, 物品 = 9 } } });
                File.WriteAllText("tshock/Chrome.Shop.json", JsonConvert.SerializeObject(shop.ShopPlugin.配置, Formatting.Indented));
            }
        }
        catch
        {
