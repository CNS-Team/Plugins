    ...
    public partial class IslandsGenerator
    {
        ...
        internal class TileBlockData
        {
            ...
            internal TileBlockData(int width, int height, string identifier = null)
            {
                ...
                this.Identifier = identifier ?? SurvivalCrisis.Rand.GenerateStr(10);
            }
            internal void AffixTo(in TileSection section, bool replaceTile = false)
            {
                if (section.Width < this.Width || section.Height < this.Height)
                {
                    throw new Exception($"空间过小: 所需({this.Width}, {this.Height})，给予({section.Width}, {section.Height})");
                }
                for (var x = 0; x < this.Width; x++)
                {
                    for (var y = 0; y < this.Height; y++)
                    {
                        var tile = this.tiles[x, y].ToTile();
                        if (section[x, y] == null)
                        {
                            section[x, y] = tile;
                        }
            ...
            internal void Save()
            {
                this.SaveTo(this.Identifier);
            }
            internal void SaveTo(string identifier)
            {
                var savePath = Path.Combine(SurvivalCrisis.IslandsPath, identifier + ".sec");
                ...
                for (var i = 0; i < this.Width; i++)
                {
                    ...
                    {
                        writer.Write(this.tiles[i, j].BTileHeader1);
                        writer.Write(this.tiles[i, j].BTileHeader2);
                        writer.Write(this.tiles[i, j].BTileHeader3);
                        writer.Write(this.tiles[i, j].STileHeader);
                        writer.Write(this.tiles[i, j].Type);
                        writer.Write(this.tiles[i, j].Wall);
                        writer.Write(this.tiles[i, j].Liquid);
                        writer.Write(this.tiles[i, j].FrameX);
                        writer.Write(this.tiles[i, j].FrameY);
                    }
                }
            }
            internal static TileBlockData FromFile(string path)
            {
                ...
                var data = new TileBlockData(width, height, file.Name[..file.Name.IndexOf('.')]);
