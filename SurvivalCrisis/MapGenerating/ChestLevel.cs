    ...
    public class ChestLevel
    {
        private struct ItemInfo
        {
            ...
            public ItemInfo(int id)
            {
                this.ID = id;
                this.Prefix = 0;
                this.MinStack = this.MaxStack = 1;
            }
            public ItemInfo(int id, int stack)
            {
                this.ID = id;
                this.Prefix = 0;
                this.MinStack = (short) stack;
            ...
            public Item ToItem()
            {
                ...
                item.stack = SurvivalCrisis.Rand.Next(this.MinStack, this.MaxStack + 1);
                return item;
            }
        }

        private readonly ItemInfo[] Accessories;
        private readonly ItemInfo[] Weapons;
        private readonly ItemInfo[] Ammo;
        private readonly ItemInfo[] Potions;
        private readonly ItemInfo[] Ores;
        private readonly ItemInfo[] Coins;
        private readonly ItemInfo[] Lighters;
        private readonly ItemInfo[] Healings;
        private readonly ItemInfo[] Tools;
        private readonly ItemInfo[] Mounts;
        private readonly ItemInfo[] Sundries;
        ...
        private ChestLevel(int level)
        {
            ...
            {
                case 1:
                    this.IdentifierL = 146;
                    this.IdentifierR = 202;
                    break;
                case 2:
                    this.IdentifierL = 193;
                    this.IdentifierR = 196;
                    break;
                case 3:
                    this.IdentifierL = 45;
                    this.IdentifierR = 177;
                    break;
                case 4:
                    this.IdentifierL = 385;
                    this.IdentifierR = 160;
                    break;
                case 5:
                    this.IdentifierL = 54;
                    this.IdentifierR = 315;
                    break;
                case 6:
                    this.IdentifierL = 195;
                    this.IdentifierR = 194;
                    break;
                case 7:
                    this.IdentifierL = 41;
                    this.IdentifierR = 541;
                    break;
            }
        }
        ...
        private List<ItemInfo[]> RandomItemTypes()
        {
            ...
            if (level == 7)
            {
                itemTypes.Add(this.Accessories);
                itemTypes.Add(this.Weapons);
            }
            else
            {
            ...
            {
                itemTypes.Add(this.Tools);
                t++;
            }
            if (SurvivalCrisis.Rand.NextDouble() < 0.05)
            {
                itemTypes.Add(this.Mounts);
                t++;
            }
            else if (level == 7)
            {
            ...
            var arr = new[] { this.Ammo, this.Potions, this.Ores, this.Coins, this.Lighters, this.Healings, this.Sundries };
        ...
        {
            var itemTypes = this.RandomItemTypes();
        ...
        public void PlaceChest(int x, int y)
        {
            ...
            var idx = WorldGen.PlaceChest(x, y, 21, style: chestType);
            try
            {
                this.AddChestItems(Main.chest[idx]);
            }
            catch (Exception e)
            {
        ...
        {
            for (var x = 0; x < block.Width - 1; x++)
            {
                for (var y = 1; y < block.Height; y++)
                {
                    if (block[x, y].type == this.IdentifierL && block[x + 1, y].type == this.IdentifierR)
                    {
                        this.PlaceChest(x + block.X, y + block.Y);
                    }
                }
            }
        }
